...
internal static class PreviewEnhancements
{
    ...
    internal static bool Handle(Forms.KeyEventArgs e, bool down)
    {
        ...
        if ((key == Forms.Keys.A || key == Forms.Keys.D) && Target.Plugin is IVideoPreviewControl { CanSeek: true })
        {
            ...
            Seek(); _nextSeek = (System.Diagnostics.Stopwatch.GetTimestamp() / (System.Diagnostics.Stopwatch.Frequency / 1000)) + EnhancementSettings.HoldDelay;
            _seekTimer = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(25) };
            _seekTimer.Tick += (_, _) =>
            {
                if (!InScope() || Target != _seekWindow || (GetAsyncKeyState((int)_seekKey) & 0x8000) == 0 ||
                    Keyboard.Modifiers != ModifierKeys.None) { StopSeek(); return; }
                if ((System.Diagnostics.Stopwatch.GetTimestamp() / (System.Diagnostics.Stopwatch.Frequency / 1000)) >= _nextSeek)
                { Seek(); _nextSeek = (System.Diagnostics.Stopwatch.GetTimestamp() / (System.Diagnostics.Stopwatch.Frequency / 1000)) + EnhancementSettings.RepeatInterval; }
            };
    ...
    internal static void StopSeek() { _seekTimer?.Stop(); _seekTimer = null; _seekWindow = null; }
